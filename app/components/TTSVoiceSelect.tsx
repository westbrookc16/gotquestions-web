...
export function TTSVoiceSelect({
  ...
  const playPreview = async (voice: string) => {
    ...
    setPlaying(voice)
    try {
      const res = await fetch(`/api/textToSpeech`, { method: "POST", body: JSON.stringify({ text: "Hello, this is a preview of the voice.", voice: voice }) })
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="text-sm text-muted-foreground">
          Voice: <span className="font-medium text-foreground">{value ? voices.find(v => v.toLowerCase() === value) : 'Not selected'}</span>
        </div>
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
        <Button onClick={() => playPreview(previewVoice)} disabled={isLoading} className="w-full sm:w-auto">
          {isLoading ? "Loading..." : "Play Preview"}
        </Button>
        <Button onClick={() => handleChange(previewVoice)} className="w-full sm:w-auto">Select Voice</Button>
        <Button variant="ghost" size="sm" onClick={() => setIsExpanded(false)} className="w-full sm:w-auto">
          Cancel
        </Button>
      </div>
    </div>
